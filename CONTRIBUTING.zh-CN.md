...
有任何疑问，欢迎提交 [issue](https://github.com/sosout/kgg/issues)，
或者直接修改提交 [MR](https://github.com/sosout/kgg/issues)!
...
随后 kgg 负责人会确认 issue 意图，更新合适的标签，关联 milestone，指派开发者。
...
- `deps: xx`: 代表 issue 跟 `dependencies` 模块相关，如 `deps: kgg-cors` 代表跟 `kgg-cors` 模块相关。
...
所有范例代码放在 [kgg/examples](https://github.com/sosout/examples) 库中。
...
如果你有仓库的开发者权限，而且希望贡献代码，那么你可以创建分支修改代码提交 MR，kgg 开发团队会 review 代码合并到主干。
...
提交后就可以在 [kgg](https://github.com/sosout/kgg/pulls) 创建 Pull Request 了。
...
- 如果功能点有新增或修改的，还需要关联文档 `doc` 和 `kgg-init` 的 PR，如 `sosout/kgg-bin#123`
...
Document change on sosout/kgg#123
...
- `next` 分支设置 tag 为 `next`，上层框架可以通过 `kgg@next` 引用开发中的版本进行测试。
- kgg 持续维护的版本以 Milestone 为准，只要是开着的版本都会进行修复。
...
[1.x milestone]: https://github.com/sosout/kgg/milestone/1
